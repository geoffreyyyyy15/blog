...
<div class="border border-top-0 border-right-0 border-secondary mt-2">
    ...
    <label for="exampleFormControlTextarea1" class="text-white" style="font-size: 10px">{{ucfirst($user->author->name) }}
        <span style="color:gray: font-weight:bold;" class="flex mt-2 text-lg"> <br>
         <a href="/post/{{$user->body}}/{{$user->id}}">{{ $user->body }}</a>
      </span>  <span class="text-green-500">-{{ $user->author->username }}</span></label></div>
